...
class Clinica:
    ...
    def mostrar_registro(self):
        for codigo in self.registro.keys():
            ...
            print(
                F'CÓDIGO: {codigo}, DADOS: {consulta.paciente.nome}, {consulta.medico.nome}, {consulta.dt_consulta}, PAGO:{consulta.pago}')
        ...
        for mes in range(1, 13):
            ...
            for consulta in self.registro.values():
                ...
                if mes == mes_consulta and consulta.pago and consulta.medico.nome == 'Dra. Vilma' and not consulta.cancelada:
                    saldo_m2 += 200
            print(f'>> Faturamento do Dr. Miguel: R$ {saldo_m1}')
            print(f'>> Faturamento da Dra. Joyce: R$ {saldo_m2}\n')
...
def agendamento_consulta(cd, clinic):
    ...
    md_nome = choice(['Dr. Miguel', 'Dra. Joyce'])
    while True:
...
def pagar_consulta(clinic):
    ...
    if reg is not None and not reg.cancelada:
        ...
        print('''Você pagou R$ 500 à clinica\n''')
        clinic.saldo = 200
        reg.medico.saldo = 300
        reg.pago = True
    elif reg is None:
